...
export const extractTimeAndSort = (a: string, b: string) => {
    ...
    const aMatches = a.match(regex), bMatches = b.match(regex);
    ...
    const aMatch = aMatches[0], bMatch = bMatches[0];
...
export const searchTextSort = (searchValues: string[]) => (a: string, b: string) => {
    ...
    const calculateMatchingChars = (values: string[], text: string) => {
        ...
        const numberOfMatchingChars =
            values.reduce((acc, c) => c.length * numberOfMatches(c, text) + acc, 0);
        // Regner ut hvor stor andel av teksten som inngår i treff i søket:
        return numberOfMatchingChars / text.length;
    };

    // Sorerer teksten hvor størst andel inngår i søketreff først
...
export const imageTagSort = (
    ...
    if (a.image?.buildTime && b.image?.buildTime) return dateSort(
        new Date(a.image.buildTime),
        new Date(b.image.buildTime)
    );
