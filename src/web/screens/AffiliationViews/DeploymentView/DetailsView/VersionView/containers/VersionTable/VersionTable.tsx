...
export const VersionTable = ({
  ...
  const data = imageTags
    ...
    .map((it) => ({
      ...
      name: versionType == ImageTagType.SEARCH
...
const HighlightedValues = (props: {searchText: string, text: string}) => {
  ...
  const indicesList: IIndices[] = searchText
      ...
      .filter(v => v.length > 0)
      .flatMap(v => {
        let startIndex = 0, index = 0, indices: IIndices[] = [];
  ...
  let mergedList: IIndices[] = [], current: IIndices = indicesList[0];
  ...
  // Formaterer resultat:
  const startOfString = mergedList[0].start > 0 ? [<>{text.slice(0, mergedList[0].start)}</>] : [];
  return <>
    {startOfString.concat(
        mergedList.flatMap((indices, i) => {
          const matchingString = <Highlight key={`h${i}`}>{text.slice(indices.start, indices.end)}</Highlight>;
          const afterMatchingString = text.slice(indices.end, mergedList[i + 1]?.start);
